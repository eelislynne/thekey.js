...
export interface WebCallback {
  (req: FastifyRequest, params:  object, middleware: Middleware): Promise<WebResponse>;
}
    ...
    return async (req: FastifyRequest, res: FastifyReply) => {
      ...
      if (middleware.length) {
        for (const mw of middleware) {
          ...
          if (!user) {
            if (mwFail) {
              this.convertResponse(res, await mwFail(req, req.params as object, middle));
            } else {
              res.code(403).send({
                success: false,
                code: 403,
      ...
      this.convertResponse(res, await cb(req, req.params as object, middle));
    };
  }
